...
const env = {
  PORT: '3007',
  DATABASE_URL: 'postgres://localhost:5432/recoveryai_insurer_sandbox',
  LOG_LEVEL: 'fatal',
};

const logger = createLogger({
  service: 'insurer-sandbox-service',
  environment: 'test',
  level: 'fatal',
  pretty: false,
});

describe('insurer-sandbox-service', () => {
  it('loads its configuration with the service name and default port', () => {
    const config = loadConfig(env);
    expect(config.SERVICE_NAME).toBe('insurer-sandbox-service');
    expect(config.PORT).toBe(3007);
  });

  it('rejects an environment with no database', () => {
    expect(() => loadConfig({ PORT: '3007' })).toThrow();
  });
  ...
  it('serves liveness', async () => {
    ...
    expect(await response.json()).toEqual({ status: 'ok', service: 'insurer-sandbox-service' });
  });
