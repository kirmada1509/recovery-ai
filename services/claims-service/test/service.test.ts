...
const env = {
  PORT: '3003',
  DATABASE_URL: 'postgres://localhost:5432/recoveryai_claims',
  LOG_LEVEL: 'fatal',
};
...
describe('claims-service', () => {
  it('loads its configuration with the service name and default port', () => {
    ...
    expect(config.SERVICE_NAME).toBe('claims-service');
    expect(config.PORT).toBe(3003);
  });

  it('rejects an environment with no database', () => {
    expect(() => loadConfig({ PORT: '3003' })).toThrow();
  });
  ...
  it('serves liveness', async () => {
    ...
    expect(await response.json()).toEqual({ status: 'ok', service: 'claims-service' });
  });
